import { GeneralField, LifeCycles, createForm } from '../src/index'
...
test('array field children state exchanges', () => {
  ...
  expect(form.query('array.0.value').take()!.value).toEqual(11)
  expect(form.query('array.1.value').take()!.value).toEqual(22)
  ...
  expect(form.query('array.0.value').take()!.value).toEqual(33)
  expect(form.query('array.1.value').take()!.value).toEqual(11)
  array.remove(1)
  ...
  expect(form.query('array.0.value').take()!.value).toEqual(33)
  expect(form.query('array.1.value').take()!.value).toEqual(44)
  expect(form.query('array.2.value').take()!.value).toEqual(55)
  array.move(0, 2)
  expect(array.value).toEqual([{ value: 44 }, { value: 55 }, { value: 33 }])
  expect(form.query('array.0.value').take()!.value).toEqual(44)
  expect(form.query('array.1.value').take()!.value).toEqual(55)
  expect(form.query('array.2.value').take()!.value).toEqual(33)
  array.move(2, 0)
  expect(array.value).toEqual([{ value: 33 }, { value: 44 }, { value: 55 }])
  expect(form.query('array.0.value').take()!.value).toEqual(33)
  expect(form.query('array.1.value').take()!.value).toEqual(44)
...
test('lazy array field query each', async () => {
  const form = attach(createForm())
  const array = attach(
    form.createArrayField({
      name: 'array',
...
test('void children', () => {
  ...
  expect(array.value).toEqual([{ aaa: 123 }])
})
...
test('array field remove memo leak', async () => {
  ...
  const form = attach(createForm({}))
  form.addEffects('defualt', () => {
    form.on(LifeCycles.ON_FORM_VALUES_CHANGE, valuesChange)
    form.on(LifeCycles.ON_FORM_INITIAL_VALUES_CHANGE, initialValuesChange)
    form.on(LifeCycles.ON_FIELD_VALUE_CHANGE, '*', handler)
  })
  const array = attach(
    form.createArrayField({
      name: 'array',
  ...
  expect(initialValuesChange).toBeCalledTimes(1)
  form.removeEffects('default')
})
...
test('void array items need skip data', () => {
  ...
  select.value = 123
  select2.value = 123
  expect(array.value).toEqual([{ space: { select: 123 } }])
  expect(array2.value).toEqual([{ select2: 123 }])
})
...
test('array field remove can not memory leak', async () => {
  ...
  form.addEffects('effect', () => {
    form.on(LifeCycles.ON_FIELD_VALUE_CHANGE, 'array.*.aa', handler)
  })
  const array = attach(
    form.createArrayField({
      name: 'array',
  ...
  const bb = attach(
    form.createField({
      ...
      reactions: (field) => {
        ...
        field.visible = field.query('.aa').take()!.value === '123'
      },
    })!
  )
  ...
  form.query('array.0.aa').take((field) => {
    // eslint-disable-next-line no-param-reassign
    ;(field as GeneralField).value = '123'
  })
  expect(bb.visible).toBeTruthy()
  expect(handler).toBeCalledTimes(1)
  form.removeEffects('effect')
})
