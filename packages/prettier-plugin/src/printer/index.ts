...
const {
  builders: { breakParent, dedent, fill, group, indent, join, line, softline, hardline, literalline },
  ...
  switch (node.type) {
    ...
    case 'element': {
      ...
      const isSelfClosingTag =
        isEmpty && (node.type === 'component' || selfClosingTags.includes(node.name) || hasSetDirectives(node))
      ...
      if (node.children) {
        ...
        let noHugSeparatorStart: _doc.builders.Line | _doc.builders.Softline | _doc.builders.Hardline | string =
          softline
        let noHugSeparatorEnd: _doc.builders.Line | _doc.builders.Softline | _doc.builders.Hardline | string = softline
        ...
        const openingTag = [
          ...
          node.name,
          indent(
            group([attributes, hugStart ? '' : !isPreTagContent(path) && !opts.bracketSameLine ? dedent(softline) : ''])
          ),
        ]

        if (hugStart && hugEnd) {
          const huggedContent = [isSingleLinePerAttribute ? hardline : softline, group(['>', body(), `</${node.name}`])]
