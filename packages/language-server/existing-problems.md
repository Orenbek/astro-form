...
## 2. our import statement regex can't match `import {} from 'xxx'` type of statement. currently it doesn't matter, only if occurs in the last line of import statement.
```ts
...
import importB from 'b';
```
## 3. if astroform directives properties defined as shorthand property, then after it doesn't support code completion.
```tsx
